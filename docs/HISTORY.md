...
| `0.9.8` | **Bug/UX fixes** — clicking a client/download row no longer crashes (`IndexOutOfBoundsException`) or loses its right-click menu (lazy-table selection resolved from the rendered `TableItem`); detail-dialog field labels no longer clip; column-selector + Link Ripper windows are resizable; dropped useless `String.intern()` from the hot display formatters/label providers (22 files) | [Release](https://github.com/vsc55/sancho-p2p/releases/tag/0.9.8) |
| `0.9.9` | **Multilingual Windows MSI** — one installer that shows its UI in the machine's Windows language (en/de/es/fr/it/ja/pt/zh), auto-selected via embedded language transforms; the associations checkbox is now localized too | [Release](https://github.com/vsc55/sancho-p2p/releases/tag/0.9.9) |
| `0.9.10` | **Installer toolchain modernized** — WiX 3.14 (end-of-life) → WiX 6 and jpackage JDK 21 → 25; the custom installer sources are WiX 4+ syntax and the multilingual step no longer needs the removed `light`/`torch`. The MSI also shrank 67.8 → 53.3 MB | [Release](https://github.com/vsc55/sancho-p2p/releases/tag/0.9.10) |
| `0.9.11` | **Stale associations are detected again** — an association left pointing at an uninstalled Sancho reported as correctly registered, so the app could neither show nor repair it (and, being under `HKCU`, it hid the machine-level entry a reinstall writes); the startup check now offers to recreate it | [Release](https://github.com/vsc55/sancho-p2p/releases/tag/0.9.11) |
