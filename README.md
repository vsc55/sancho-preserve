...
| `0.9.6` | **Windows associations: status + startup check** — the *Windows Registry* page shows each protocol/`.torrent` association's current state (Sancho / another app, user vs. machine); a startup check offers to create missing ones (or stop asking); toggle in Preferences | [Release](../../releases/tag/0.9.6) |
