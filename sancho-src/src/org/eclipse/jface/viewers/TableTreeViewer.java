...
public class TableTreeViewer extends AbstractTreeViewer {
	...
	protected void doUpdateItem(Item item, Object element) {
		...
		for (int column = 0; column < columnCount || column == 0; column++) {
			...
			// Modern JFace dropped the applyFontsAndColors(TableTreeItem) overload
			// together with the widget; apply to the backing TableItem instead.
			getColorAndFontCollector().applyFontsAndColors(ti.getTableItem());
		}
	...
	protected Item getChild(Widget widget, int index) {
		if (widget instanceof TableTreeItem) {
			// Repo's patched TableTreeItem replaced getItem(int) with getItem(TableItem).
			return ((TableTreeItem) widget).getItems()[index];
		}
